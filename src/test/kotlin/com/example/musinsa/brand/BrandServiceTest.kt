...
class BrandServiceTest : BehaviorSpec({
    ...
    given("새로운 브랜드 생성 요청이 고유한 이름으로 들어오면") {
        ...
        val brandDto = createBrandRequest.toBrandDto()
...
//        val existingBrandDto = BrandDto(id = brandId, name = "기존 브랜드")
//        val updatedBrandDto = BrandDto(id = brandId, name = updateBrandRequest.name)
...
//                verify(exactly = 1) { brandDomainService.save(any()) }
//            }
//        }
//    }
    ...
    given("존재하는 브랜드를 삭제할 때") {
        val brandId = 1L
        val existingBrandDto = BrandDto(id = brandId, name = "삭제할 브랜드")
